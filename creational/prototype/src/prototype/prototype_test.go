...
func TestClone(t *testing.T) {
	shirtCache := p.GetShirtsCloner()
	if shirtCache == nil {
		t.Fatal("Received cache was nil")
	}

	item1, err := shirtCache.GetClone(p.White)
	if err != nil {
		t.Error(err)
	}

	if item1 == p.WhitePrototype {
		t.Error("item1 cannot be equal to the white prototype")
	}

	shirt1, ok := item1.(*p.Shirt)
	if !ok {
		t.Fatal("Type assertion for shirt1 couldn't be done successfully")
	}
	shirt1.SKU = "abbcc"

	item2, err := shirtCache.GetClone(p.White)
	if err != nil {
		t.Fatal(err)
	}

	shirt2, ok := item2.(*p.Shirt)
	if !ok {
		t.Fatal("Type assertion for shirt 1 couldn't be done successfully")
	}
